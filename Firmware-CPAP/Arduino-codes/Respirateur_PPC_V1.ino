...
int configuration = 0; //Variable de la configuration de la vitesse
...
void setup() {
  ...
  Airflow_Timer();
  Pressure_Timer();
  Speed = map(speedBlower, 0, 1800, 0, 100); // Calcul du pourcentage de la vitesse du blower
  lcd.setCursor(0, 2);
  lcd.print("Vitesse : ");
  lcd.print(Speed);
  lcd.print(" %");
...
void expiration() {
  ...
  if (configuration == 1 || configuration == 2) { //condition de configuration de la vitesse
    ...
    if (speedBlower <= 0)  speedBlower = 0; //Limitation de la valeur min
    ...
    lcd.print(Speed);
    lcd.print(" %");
    if (configuration == 2) { //condition de changement de la vitesse
