...
int configuration = 0; //Variable de la configuration du débit d'air
int seuil = 20; //Seuil du débit d'air à atteindre par le respirateur PPC
int asservissement = 0; //Variable d'asservisement du blower
...
void setup() {
  ...
  pinMode(PC13, INPUT_PULLUP); //User button
  sensor_Init();
  PWM_Blower();
  Airflow_Timer();
  Pressure_Timer();
...
int airFlow() { // Cette fonction va permettre de lire le débit d'air mesuré par le débitmètre, elle va être activée dans un timer configuré
  myWire1.begin();
...
void expiration() {
  ...
  if ((millis() - lastDebounceTime3) > debounceDelay3) {
    ...
    if (reading3 != buttonState3) {
      ...
      if (buttonState3 == 0 && configuration == 0) {
        configuration = 1;
        hardwareTimer1->setCaptureCompare(TIM_CHANNEL_ESC_BLOWER, BlowerSpeed2MicroSeconds(0), MICROSEC_COMPARE_FORMAT); // Arrête le blower pendant la configuration
      }
      else if (buttonState3 == 0 && configuration == 1) {
        configuration = 2;
      }
    }
  }
  ...
  if (configuration == 1) {
    ...
    if (seuil <= 0)  seuil = 0; //Limitation de la valeur min
    if (seuil >= 38) seuil = 38; //Limitation de la valeur max
    lcd.setCursor(0, 2);
    lcd.print("Seuil : ");
    lcd.print(seuil);
    lcd.print(" SLPM");
  }
  else if (configuration == 2) { //condition de la mise en place du débit d'air à atteindre
    while (seuil != airFlow()) { //Boucle modifiant la vitesse jusqu'à atteindre le seuil configuré
      if (seuil > airFlow()) speedBlower++;
      if (seuil < airFlow()) speedBlower--;
      hardwareTimer1->setCaptureCompare(TIM_CHANNEL_ESC_BLOWER, BlowerSpeed2MicroSeconds(speedBlower), MICROSEC_COMPARE_FORMAT);
...
void PWM_Blower() { // Cette fonction va permettre de générer le signal PPM qui va contrôler le blower, cette génération est sous la forme d'un timer pour créer un signal PWM
  ...
  hardwareTimer1->setCaptureCompare(TIM_CHANNEL_ESC_BLOWER, BlowerSpeed2MicroSeconds(speedBlower), MICROSEC_COMPARE_FORMAT); //Définit le rapport cyclique de la PWM
